                ...
                {loading ? (
                    ...
                    <div className="space-y-4">
                        {myAds.length > 0 ? myAds.map((ad) => (
                            <div key={ad.id} className="bg-white border border-gray-300 rounded overflow-hidden flex flex-col md:flex-row p-4 gap-4 shadow-sm relative group">
                                
                                {/* Ad Image */}
                                <div className="w-full md:w-48 h-32 bg-gray-100 rounded overflow-hidden shrink-0">
                                    <img src={ad.adImages?.[0]} className="w-full h-full object-cover" alt="ad" />
                                </div>
                                ...
                                <div className="flex-1 flex flex-col justify-between">
                                    <div>
                                        ...
                                        <p className="text-xl font-bold text-[#002f34] mt-1">Rs {ad.adPrice}</p>
                                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                                            <span>Active from: {ad.createdAt?.toDate().toLocaleDateString() || "Today"}</span>
                                            <span className="bg-blue-100 text-blue-600 px-2 py-0.5 rounded font-bold uppercase">Active</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-4 md:mt-0 md:justify-end items-center">
                                        ...
                                        <button className="px-6 py-2 bg-[#002f34] text-white font-bold rounded hover:bg-[#002f34]/90 transition-all text-sm">
                                            Sell faster now
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="bg-white p-20 text-center border rounded">
                                ...
                                <button onClick={() => navigate('/post')} className="mt-4 text-blue-600 font-bold underline">Post an ad now</button>
                            </div>
                        )}
                    </div>
                )}
