...
function ChatPage() {
  ...
  return (
    // Height ko screen ke mutabiq fix kiya taake input nichay na jaye
    <div className="flex flex-col h-[calc(100vh-145px)] bg-white border-l border-gray-200 overflow-hidden relative">
      ...
      <div className="p-3 border-b border-gray-200 flex items-center bg-white shrink-0 z-10 shadow-sm">
        ...
        <div>
          <h4 className="font-bold text-[#002f34] text-sm">{adInfo.seller}</h4>
      ...
      {/* 3. Messages Area */}
      <div className="flex-1 overflow-y-auto bg-[#f2f4f5] p-4 flex flex-col gap-2 no-scrollbar">
        {messages.map((msg) => (
          <div key={msg.id} className={`max-w-[85%] p-2.5 rounded-lg text-[13px] shadow-sm ${msg.senderId === auth.currentUser?.uid ? "bg-[#e2f7ff] self-end rounded-tr-none text-[#002f34]" : "bg-white self-start rounded-tl-none text-[#002f34] border border-gray-100"}`}>
            {msg.text}
          </div>
        ))}
        <div ref={scrollRef} />
      ...
      {/* 4. Input Section - Sticky Bottom taake hamesha dikhay */}
      <div className="bg-white border-t sticky bottom-0 left-0 right-0 z-20">
        {/* Tabs: Questions & Offer */}
